...
class ReaderROOT(Input):
    ...
    def _load_next_file(self):
        ...
        if not self._tree:
            sys.exit(
                f"ERROR: in ReaderROOT {self.name}, tree {self.config['tree']} could not be found in file {self._f}"
            )
        ...
        if "variables" in self.config:
            ...
            for _, branch in self._variables.items():
                if branch not in branches:
                    sys.exit(
                        f"ERROR: in ReaderROOT {self.name}, {branch} could not be found in tree {self.config['tree']}"
                    )
        else:
        ...
        if "timestamp" in self.config:
            ...
            store_name = self._output_format.format(
                name=self.name, variable=timestamp_var
            )
            if (
                timestamp_var not in self._variables
                and store_name not in self._variables
            ):
                if timestamp_var not in branches:
                    sys.exit(
                        f"ERROR: in ReaderROOT {self.name}, timetamp variable set to {timestamp_var} but it wasn't found in the tree {self.config['tree']}."
                    )
        ...
        if "objects" in self.config:
            ...
            for obj_store_name, path in obj_vars.items():
                ...
                if isinstance(obj, ROOT.TDirectory):
                    ...
                    self._obj_variables.update(
                        {
                            self._output_format.format(name=self.name, variable=k): k
                            for k in dir_objs
                        }
                    )
                elif not obj:
                    sys.exit(
                        f"ERROR: in ReaderROOT {self.name}, can't find {path} in file {self._files[self._files_index]}."
                    )
                else:
    ...
    def _parse_config_list(self, config):
        ...
        for variable in config:
            if type(variable) == str:
                ...
                # var name
                store_name = self._output_format.format(
                    name=self.name, variable=variable
                )
                ...
                for var_name, path in variable.items():
                    store_name = self._output_format.format(
                        name=self.name, variable=var_name
                    )
    ...
    def _get_total_events(self):
        ...
        for file_name in self._files:
            try:
                ...
                sys.exit(
                    f"ERROR: in ReaderROOT {self.name}, file {file_name} couldn't be opened.\n"
                    "Please ensure the path and name is correct, and the file is of the correct type."
                )
            tree = f.Get(self.config["tree"])
            if not tree:
                sys.exit(
                    f"ERROR: in ReaderROOT {self.name}, tree {self.config['tree']} could not be found in file {file_name}"
                )
    ...
    def get_event(self, skip=False):
        ...
        for store_name, branch_name in self._variables.items():
            ...
            if iterable(value):
                ...
                value = np.array(
                    list(value), copy=False
                )  # copy=False? might cause seg fault?
