...
parser.add_argument(
    "--event_num",
    "-n",
    help="Specify how many events to process (optional).",
    required=False,
    default=None,   # If not passed in
    action="store", # store internally
)
...
parser.add_argument(
    "--event_start",
    "-s",
    help="Specify the first event number (optional).",
    required=False,
    default=None,   # If not passed in
    action="store", # store internally
)
...
if __name__ == "__main__":
    # Check if PYRATE is in the environment, if not add it
    if "PYRATE" not in os.environ:
        FN.add_pyrate_to_env()
    ...
    for j_file in args.job_config:
        ...
        job = Job(j_name, j_config, j_log, args)
